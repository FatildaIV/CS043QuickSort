...
*/
        ...
        {
            QuickSort<T>(ref pole, 0, pole.Length - 1);
        }

        private static void QuickSort<T>(ref T[] pole, int indexL, int indexR)
        {
            if (indexL < indexR)
            {
                int pivot = Partition(ref pole, indexL, indexR);

                if (pivot > 1)
                {
                    QuickSort<T>(ref pole, indexL, pivot - 1);
                }
                if (pivot + 1 < indexR)
                {
                    QuickSort<T>(ref pole, pivot + 1, indexR);
                }
            }

        }

        private static int Partition<T>(ref T[] pole, int indexL, int indexR)
        {
            T pivot = pole[indexL];
            while (true)
            {

                while ((dynamic)pole[indexL] < pivot)
                {
                    indexL++;
                }

                while ((dynamic)pole[indexR] > pivot)
                {
                    indexR--;
                }
                ...
                if (indexL < indexR)
                {
                    if ((dynamic)pole[indexL] == (dynamic)pole[indexR]) return indexR;

                    StaticProhoditPromenna<T>(ref pole[indexL], ref pole[indexR]);
                    
                    //int temp = pole[indexL];
                    //pole[indexL] = pole[indexR];
                    //pole[indexR] = temp;
                }
                else
                {
